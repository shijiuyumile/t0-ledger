...
```
总盈亏 = 移动均价已实现 + 持仓浮动盈亏 + 股息 + 利息
```

已实现按券商常见**移动加权成本**（清仓成本归零后再买重新起算），与涨乐累计收益对齐；**不随** `matchMode` 变化。  
价格/时间配对净利仅作「做T分析」副指标（`matchRealized`）。

持仓浮动优先用对账单；切「做T时间」不改账户总盈亏。

另有对照口径：`总资产 − 银证净入金`（资金口径，通常≠涨乐「累计收益」）。
...
改「最新价」走 `setRefQuote`，台账/卖出/待回补/关注价全页 `renderAll` 联动。

待回补：按标的分卡（与台账同款色条/品种底色）。  
亏损卖出/关注价：醒目「扳本」徽章 + 回本卖出价估算。
...
本 APP：`移动均价已实现 + 持仓浮动 + 股息利息`（对齐涨乐成本口径）。

- **持仓浮动**必须用对账单「股票持仓」合计（含 **qty=0 但仍有持仓盈亏** 的已清仓行）。漏掉会把总盈亏抬高一千多。
- 字段 `account.holdPnlTotal` = 账单合计行持仓盈亏；计算时优先用它。
- 账单 `asOf` 偏旧、涨乐用实时价时，浮动部分仍可能差几百～一两千，属正常缝。
- 「做T配对净利」（价格/时间核销）专门做 T 复盘，**不要**拿去和涨乐总收益比。
