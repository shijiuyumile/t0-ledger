...
- 卖单与同代码**剩余买单**配对，**不限是否同一天**。
- 配对规则「价格最相近」：
  1. 优先：买价 ≤ 卖价 中，买价最高的；
  2. 否则：买价 > 卖价 中，买价最低的；
  3. 数量不够则级联下一笔。
- 盈亏 = Σ(卖价−买价)×数量 − 卖出费用分摊 − 买入费用分摊。
- 整笔卖单净利 > 0 → 「做T成功」；≤ 0 → 「亏损卖出」。
- 支持同日先卖后买回。
- **做T时间口径**（可选）：`computeMatches(trades, { fromDate })`
  - `fromDate` 之前的买单 = **底仓**（`lot.isBase`），不参与配对；
  - `fromDate` 之前的卖单不计入该口径核销结果；
  - 台账默认模式为「整体持仓」（全历史，与旧版一致）。
...
持仓浮动优先用对账单「股票持仓」里的成本价/现价/持仓盈亏；用户可手填现价覆盖。

另有对照口径：`总资产 − 银证净入金`（与券商 APP 数字可能仍有差异，属正常）。

**账户总盈亏始终用全历史配对**，不随「做T时间」切换而变。
